...
export async function POST(request: Request) {
  try {
    const ctx = await requireApiPermission(request, "customers:read");
    const body = (await request.json()) as Record<string, unknown>;
    const data = await getOpsServices().listCustomers({
      ...body,
...
export async function PUT(request: Request) {
  try {
    const ctx = await requireApiPermission(request, "customers:write");
    const body = (await request.json()) as Record<string, unknown>;
    const data = await getOpsServices().upsertCustomer({
      ...body,
      organizationId: body.organizationId ?? ctx.organizationId,
